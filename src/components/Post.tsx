...
import { Star, MessageCircle, Share2, ThumbsUp, Calendar, Clock, Trophy, MoreVertical, Bookmark, Award, UserPlus, CheckCircle, XCircle, X, Pencil, Trash2 } from 'lucide-react';
...
export default function Post({ post, onOpenBetModal, onStatusChange, onPostDeleted }: PostProps) {
  ...
  const handleMenuAction = async (action: string) => {
    ...
    switch (action) {
      ...
      case 'save':
        ...
        break;
      case 'markWin':
        if (onStatusChange) onStatusChange(post.id, 'won');
        break;
      case 'markLoss':
        if (onStatusChange) onStatusChange(post.id, 'lost');
        break;
      case 'rate':
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {post.user.badge && <Badge type={post.user.badge} />}
            </div>
                  ...
                  {isOwner && (
                    <>
                      <button
                        ...
                        className="w-full p-3 text-left hover:bg-gray-100 rounded-lg flex items-center space-x-3 transition-colors text-blue-600"
                      >
                        <Pencil className="h-5 w-5" />
                        <span>Modifier le pronostic</span>
                      </button>
                      <button
                        onClick={() => handleMenuAction('delete')}
                        className="w-full p-3 text-left hover:bg-red-50 rounded-lg flex items-center space-x-3 transition-colors text-red-600"
                      >
                        <Trash2 className="h-5 w-5" />
                        <span>Supprimer le pronostic</span>
                      </button>
                    </>
                  )}
                  <button
